...
public class GameService implements IGameService {
  ...
  @Override
  public GameBeans start(String playerOneName) {
    Player playerOne = playerService.createNewPlayer(playerOneName, PLAYER_ONE_NUM);
    Game game = saveGame(new Game(playerOne));
    return modelMapper.map(game, GameBeans.class);
  }

  @Override
  public String load(UUID gameId,String playerTwoName) {
    Game game = getGame(gameId);
    Player playerTwo = playerService.createNewPlayer(playerTwoName, PLAYER_TWO_NUM);
    game.setPlayerTwo(playerTwo);
    game = saveGame(game);

    GameBeans gameBeans = modelMapper.map(game, GameBeans.class);;
